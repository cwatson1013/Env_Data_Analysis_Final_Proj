...
mk.test(carbon.paul$doc[1:137]) #negative z score indicates a negative trend; p-value less than 0.05, so reject the null that there is no trend in the data and do another pettitt test
mk.test(carbon.paul$doc[138:373]) #positive z-score indicates there's a positive trend  

#mk.test in Tuesday lake
mk.test(carbon.tuesday$doc) #positive z-score indicates a positive trend in the data and the p-value is very low, so we reject the null that there is no trend in the data
...
Decided to conduct a Mann-Kendall test on Peter, Paul, and West Long Lake because those are the lakes that had continuous samping as shown in \autoref{fig:fig9}. 
```{r, echo=FALSE, eval=TRUE fig.height = 6, fig.width = 6, warning = FALSE, fig.cap="\\label{fig:fig10} Dissolved organic carbon concentration over time with year on the x-axis."}

#graph of doc concentration over time
ggplot(carbon.data.skinny, aes(x = Date, y = doc, color = Lake.Name)) + 
  geom_point(alpha=0.5, size = 1) +
  labs(x = "Date", y = "Dissolved Organic Carbon", color = "Lake Name") +
  geom_vline(xintercept = as.Date("2000-08-23", origin = "1970-01-01"), color="#253494", lty = 2) +  #change point at row 1628
  geom_vline(xintercept = as.Date("2006-08-21", origin = "1970-01-01"), color="#7fcdbb", lty = 2) +  #change point at 2419
  ...
  grid arrange to get one large graph and then do the lakes seperate 
  Peter, Paul, Tuesday, West, and East lakes for the Pettit and Man-Kendall test 
```
